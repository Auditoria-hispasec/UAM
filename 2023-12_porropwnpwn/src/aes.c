// Fusilado de https://gist.github.com/acapola/d5b940da024080dfaf5f
...
size_t aes128_enc_ecb(int8_t *plainText,int8_t *cipherText, size_t len) {
    for (int i = 0; i < len; i += 16) {
        aes128_enc(plainText + i, cipherText + i);
    }
    return len + (16 - (len % 16));
}

size_t aes128_dec_ecb(int8_t *cipherText,int8_t *plainText, size_t len) {
    for (int i = 0; i < len; i += 16) {
        aes128_dec(cipherText + i, plainText + i);
    }
    return len + (16 - (len % 16));
}
